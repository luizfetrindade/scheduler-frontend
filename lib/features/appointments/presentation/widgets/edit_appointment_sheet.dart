...
class _EditAppointmentSheetState extends State<EditAppointmentSheet> {
  ...
  void initState() {
    super.initState();
    _selectedDateTime = widget.appointment.startsAt;
    _nameController.text = widget.appointment.clientName;
    _notesController.text = widget.appointment.notes ?? '';
    _selectedServiceId = widget.appointment.serviceId;

    final durationMinutes = widget.appointment.endsAt
        .difference(widget.appointment.startsAt)
        .inMinutes;
    if (_durationOptions.contains(durationMinutes)) {
      _durationMinutes = durationMinutes;
      _isCustomDuration = false;
    } else {
      _durationMinutes = durationMinutes;
      _isCustomDuration = true;
      _customDurationController.text = durationMinutes.toString();
    }
  }
  ...
  void _submit() {
    ...
    if (widget.appointment.isRecurring) {
      setState(() => _isSubmitting = false);
      _showRecurrenceEditDialog(context);
    } else {
      _dispatchUpdate(context, updateFuture: false);
    }
  }

  void _showRecurrenceEditDialog(BuildContext context) {
    showDialog<void>(
      context: context,
      builder: (dialogContext) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              Navigator.of(dialogContext).pop();
              _dispatchUpdate(context, updateFuture: true);
            },
            child: Text(
  ...
  void _dispatchUpdate(BuildContext context, {required bool updateFuture}) {
    if (!mounted) return;
    ...
    final originalDuration = widget.appointment.endsAt
        .difference(widget.appointment.startsAt)
        .inMinutes;

    final dateTimeChanged = _selectedDateTime != widget.appointment.startsAt;
    final durationChanged = duration != originalDuration;
    final nameChanged =
        _nameController.text.trim() != widget.appointment.clientName;
    final notesText = _notesController.text.trim();
    final notesChanged = notesText != (widget.appointment.notes ?? '');
    final serviceChanged = _selectedServiceId != widget.appointment.serviceId;

    final hasChanges = dateTimeChanged ||
        durationChanged ||
        nameChanged ||
        notesChanged ||
        serviceChanged;
    ...
    if (widget.appointment.isRecurring) {
      context.read<ScheduleBloc>().add(
            ScheduleRecurrenceUpdateRequested(
              appointmentId: widget.appointment.id,
              startsAt: dateTimeChanged ? _selectedDateTime : null,
              durationMinutes: durationChanged ? duration : null,
              clientName: nameChanged ? _nameController.text.trim() : null,
              notes: notesChanged ? notesText : null,
              clearNotes: notesChanged && notesText.isEmpty,
              serviceId: serviceChanged ? _selectedServiceId : null,
              clearService: serviceChanged && _selectedServiceId == null,
              updateFuture: updateFuture,
            ),
          );
    } else {
      context.read<ScheduleBloc>().add(
            ScheduleAppointmentUpdateRequested(
              appointmentId: widget.appointment.id,
              startsAt: dateTimeChanged ? _selectedDateTime : null,
              durationMinutes: durationChanged ? duration : null,
              clientName: nameChanged ? _nameController.text.trim() : null,
